#!/bin/bash
# am-outcome - Track outcomes of completed issues for emergent learning

show_help() {
    echo "am-outcome - Track outcomes for emergent learning"
    echo ""
    echo "Usage:"
    echo "  am-outcome record <issue> [options]   Record outcome when closing issue"
    echo "  am-outcome show <issue>               Show outcome for specific issue"
    echo "  am-outcome stats [options]            Show aggregate statistics"
    echo "  am-outcome recent [n]                 Show n most recent outcomes"
    echo ""
    echo "Record options:"
    ...
    echo ""
    echo "Examples:"
    echo "  am-outcome record issue-abc --success --approach implement-iterate"
    echo "  am-outcome record issue-xyz --failure --approach big-bang"
    echo "  am-outcome stats --by-approach"
    exit 0
}
...
usage() {
    echo ""
    echo -e "${BOLD}am-outcome - Track outcomes for emergent learning${NC}"
    echo ""
    echo "Record:"
    echo "  am-outcome record <issue> [--success|--failure] [--approach name] [--complexity low|medium|high]"
    echo ""
    echo "View:"
    echo "  am-outcome show <issue>           Show outcome for specific issue"
    echo "  am-outcome stats                  Show aggregate statistics"
    echo "  am-outcome stats --by-approach    Stats grouped by approach"
    echo "  am-outcome stats --by-tag         Stats grouped by tag"
    echo "  am-outcome stats --by-complexity  Stats grouped by complexity"
    echo "  am-outcome recent [n]             Show n most recent outcomes"
    echo ""
    echo "Examples:"
    echo "  am-outcome record impact-abc --success --approach research-first --complexity medium"
    echo "  am-outcome stats --by-approach"
    echo ""
}
...
case "${1:-}" in
    ...
    record)
        ...
        if [ -z "$issue" ]; then
            echo "Usage: am-outcome record <issue> [options]"
            exit 1
        fi
