#!/bin/bash
# am-context - Rich context trails for beads issues
#
# Enhanced with: findings, decisions, tags, related issues, blockers

show_help() {
    echo "am-context - Rich context trails for beads issues"
    echo ""
    echo "Usage:"
    echo "  am-context <issue>                    Show all context for issue"
    echo "  am-context <issue> --add \"text\"       Add a note"
    echo "  am-context <issue> --add-finding \"text\" [--source src] [--confidence high|medium|low]"
    echo "  am-context <issue> --add-decision \"what\" --why \"reason\" [--alternatives \"a,b,c\"]"
    echo "  am-context <issue> --add-tag \"tag1,tag2\""
    echo "  am-context <issue> --link \"other-issue\""
    echo "  am-context <issue> --resolved \"blocker\" --resolution \"how fixed\""
    echo ""
    echo "Search:"
    echo "  am-context --search \"keyword\"        Search across all context"
    echo "  am-context --tag \"tagname\"           Find issues by tag"
    echo "  am-context --list                    List all issues with context"
    echo ""
    echo "Options:"
    echo "  -h, --help                           Show this help message"
    echo ""
    echo "Examples:"
    echo "  am-context issue-abc --add \"Discovered the API rate limits\""
    echo "  am-context issue-abc --add-finding \"Redis is bottleneck\" --confidence high"
    echo "  am-context issue-abc --add-decision \"Use PostgreSQL\" --why \"Better for our scale\""
    exit 0
}
...
if not issue_ctx:
    ...
    print("    am-context $issue --add \"note\"")
    print("    am-context $issue --add-finding \"discovery\"")
    print("    am-context $issue --add-decision \"what\" --why \"reason\"")
    sys.exit(0)
...
usage() {
    echo ""
    echo -e "${BOLD}am-context - Rich context trails for beads issues${NC}"
    echo ""
    echo "View context:"
    echo "  am-context <issue>                Show all context"
    echo "  am-context <issue> --findings     Show only findings"
    echo "  am-context <issue> --decisions    Show only decisions"
    echo ""
    echo "Add context:"
    echo "  am-context <issue> --add \"text\"                       Add note"
    echo "  am-context <issue> --add-finding \"text\" [--source s] [--confidence high|medium|low]"
    echo "  am-context <issue> --add-decision \"what\" --why \"reason\" [--alternatives \"a,b\"]"
    echo "  am-context <issue> --add-tag \"tag1,tag2\""
    echo "  am-context <issue> --link \"other-issue\""
    echo "  am-context <issue> --resolved \"blocker\" --resolution \"how fixed\""
    echo ""
    echo "Search:"
    echo "  am-context --search \"keyword\""
    echo "  am-context --tag \"tagname\""
    echo "  am-context --list"
    echo ""
}
